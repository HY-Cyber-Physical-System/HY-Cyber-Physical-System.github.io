...
(async()=>{
 for(const engine of ['chromium','webkit']){
  ...
  for(const software of [false,true]){
   ...
   const idleBefore=await canvas.screenshot();await page.waitForTimeout(800);const idleAfter=await canvas.screenshot();
   assert.ok(!idleBefore.equals(idleAfter),'Idle motion must continue after the entrance animation ends');
   assert.equal(await page.evaluate(()=>window.scrollY),0,'Idle animation must not need scrolling');
